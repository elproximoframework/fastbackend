...
def seed():
    for db_url in DB_URLS:
        ...
        try:
            ...
            cur = conn.cursor()
            print(f"[{db_name}] Seeding videos...")
            ...
            for v in videos:
                cur.execute(
                    "INSERT INTO youtube (video_name, url, type, description, description_en, date) VALUES (%s, %s, %s, %s, %s, %s) ON CONFLICT (url) DO NOTHING",
                    v
                )
                
            print(f"[{db_name}] Seeded {len(videos)} videos successfully")
            cur.close()
            conn.close()
        except Exception as e:
            print(f"[{db_name}] Error seeding: {e}")
