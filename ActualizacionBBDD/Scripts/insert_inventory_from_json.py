...
def upsert_inventory_item(cur, item):
    """
    Upserts an inventory item into the spacex_inventory table.
    ...
    try:
        ...
        cur.execute("SELECT id FROM spacex_inventory WHERE slug = %s", (slug,))
        exists = cur.fetchone()

        if exists:
            # UPDATE: Quote identifiers for safety
            set_clauses = [sql.SQL("{} = %s").format(sql.Identifier(k)) for k in cols]
            query = sql.SQL("UPDATE spacex_inventory SET {} WHERE slug = %s").format(
            ...
            # INSERT
            query = sql.SQL("INSERT INTO spacex_inventory ({}) VALUES ({})").format(
    ...
    try:
        ...
        for label, url in [("Local", LOCAL_URL), ("Remote", REMOTE_URL)]:
            ...
            try:
                ...
                cur = conn.cursor()
                for item in items:
                    upsert_inventory_item(cur, item)
                conn.commit()
                cur.close()
                conn.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Upsert SpaceX inventory items from JSON into Local and Remote databases.")
