...
def upsert_news_item(cur, item):
    """
    Upserts a news item into the news table.
    ...
    try:
        ...
        if exists:
            ...
            query = sql.SQL("UPDATE news SET {} WHERE slug = %s").format(
            ...
            # INSERT
            query = sql.SQL("INSERT INTO news ({}) VALUES ({})").format(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Upsert news items from JSON into Local and Remote databases.")
