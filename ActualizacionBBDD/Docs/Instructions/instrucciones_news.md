...
| `rutanoticia` | String  | Ruta completa al archivo (ej:`noticia1.md`).                                             |
| `timestart`   | Integer | Segundo de inicio en el vídeo (opcional).                                                 |
| `show`        | Boolean | `True` por defecto.                                                                      |

## 2. Instrucciones de Extracción

Para cada noticia, tendrás dos archivos: `archivo.md` (español) y `archivo_en.md` (inglés). se guardara en la carpeta D:\YoutubeElProximoFrameworkEnElEspacio\Web\Cloudinary\news
...
5. **Categoría y Lugar:** Identificar la entidad principal (SpaceX, NASA, China, etc.) y asignar la categoría y ubicación correspondientes según la tabla del punto 1.
6. **Featured y YouTube:** Si se proporciona un enlace de YouTube (`linkyoutube`), el campo `featured` debe establecerse automáticamente en `True`.
7. **Timestart:** Si se proporciona un archivo o lista con los tiempos de inicio por noticia, deberás incluir el valor `timestart` correspondiente para esa noticia.
8. **Imagen:**
   * **Creación de Imágenes:** No se va a crear imagen de forma automatica, sera el usuario el que busque la imagen y la pegue en la carpeta corresponiente con el nombre indicado (siguiente paso).
...
9. **Etiquetas:** Generar una lista de 3 a 5 palabras clave relevantes para el contenido.
10. **Ruta Noticia:** Usar siempre la ruta completas sera el nombre del archivo en español (ej: `260328_informe.md`). 
11. **Link Noticia:** Si se proporciona un enlace externo de la noticia original, guárdalo opcionalmente.
